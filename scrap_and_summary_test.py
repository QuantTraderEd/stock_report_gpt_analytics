import os
import re
import datetime as dt
import traceback

import requests
import pandas as pd

from tqdm import tqdm
from bs4 import BeautifulSoup

from utils import make_requests


def main(api_key: str, organization: str):
    """
    증권사 리포트 스크랩핑 & OpenAI API 요약
    :param api_key: OpenAI API 키
    :param organization: OpenAI API organization
    """

    base_url = 'https://finance.naver.com/research/'
    list_url = base_url + "company_list.naver"

    ## 1page에 있는 url들을 크롤링함
    response = requests.get(list_url)
    html = response.text
    soup = BeautifulSoup(html, 'html.parser')
    report_raw_tags = soup.select('#contentarea_left > div > table > tr > td > a')

    report_urls = []
    for report_raw_tag in report_raw_tags:
        if report_raw_tag.attrs['href'].startswith('company'):
            report_urls.append(base_url + report_raw_tag.attrs['href'])
    ###############################################################################

    # 각 url에서 그날의 레포트를 크롤링함
    datetime_utc = dt.datetime.utcnow()
    datetime_kst = datetime_utc + dt.timedelta(hours=9)
    today = datetime_kst.today().date().strftime('%Y.%m.%d')
    today_7_ago = datetime_kst.today().date() - dt.timedelta(days=7)
    today_7_ago = today_7_ago.strftime('%Y.%m.%d')

    folder_path = today.replace('.', '/')
    folder_path_1_ago = today_7_ago.replace('.', '/')

    os.makedirs(folder_path, exist_ok=True)

    reports = []
    for report_url in tqdm(report_urls):
        report_response = requests.get(report_url)
        report_html = report_response.text
        report_soup = BeautifulSoup(report_html, 'html.parser')

        tags = report_soup.select('#contentarea_left > div > table > tr')
        report_text = ''
        target_company = ''
        for i, tag in enumerate(tags):
            if i == 0:
                source_text = tag.find('p', attrs={'class': 'source'}).text
                source_text_list = source_text.split('|')
                target_company = tag.find('span').text
            tag_text = tag.text.replace('\t', '').lstrip().rstrip()
            report_text += tag_text + '\n'
        report_day = re.search(r"\d{4}\.\d{2}\.\d{2}", report_text).group()
        report_text = report_text.replace('|', ' ')
        if report_day < today_7_ago:
            break
        broker_name = source_text_list[0]
        report = [report_day, broker_name, target_company, report_text]
        reports.append(report)

    df_reports = pd.DataFrame(reports, columns=['date', 'broker_name', 'target_company', 'text'])
    df_reports.to_csv(folder_path + '/reports.csv', sep='|', index=False)

    ############################################################################
    base_prompt = """
    The text below is a stock report on a company.
    Please summarize it in Korean.
    
    The format is as follows
    ## Company name
    - Summary
    
    리포트:
    """
    summary = [f'# {today} stock report']

    for report in tqdm(reports):
        report_text = report[-1]
        prompt = base_prompt + report_text
    #     try:
    #         chat_gpt_response = make_requests(engine="gpt-3.5-turbo-0301",
    #                                           prompts=prompt,
    #                                           api_key=args.api_key,
    #                                           organization=args.organization)
    #         summary.append(chat_gpt_response["choices"][0]["message"]["content"])
    #     except Exception as e:
    #         msg = traceback.format_exc()
    #         print(msg)
    #         break
    #
    # with open(os.path.join(folder_path, 'README.md'), 'w') as file:
    #     file.write('\n'.join(summary))
    #
    # with open('README.md', 'w') as file:
    #     file.write('\n'.join(summary))


if __name__ == '__main__':
    import argparse

    parser = argparse.ArgumentParser()
    parser.add_argument(
        "--api_key",
        type=str,
        default="",
    )
    parser.add_argument(
        "--organization",
        type=str,
        default="",
    )
    args = parser.parse_args()

    main(args.api_key, args.organization)

